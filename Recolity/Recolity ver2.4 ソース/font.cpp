...
void DirectWrite::DrawString(std::string str, D2D1_RECT_F rect, D2D1_DRAW_TEXT_OPTIONS options)
{
   // pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF(coler)), &pSolidBrush);
	...
	// 描画処理
	pRT->DrawText(wstr.c_str(), (UINT)wstr.size(), pTextFormat, rect, pSolidBrush, options);
...
void DirectWrite::TextInit()
{
	...
	// バックバッファの取得
	// 型：IDXGISwapChain
	GetSwapChainrend()->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));
