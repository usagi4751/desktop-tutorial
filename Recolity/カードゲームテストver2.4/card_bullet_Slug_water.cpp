...
#define RANGE (1000)
void Bullet_Slug_Water::Use(int rank) {
...
void Bullet_Slug_Water::DrawTextPreview(D2D1_RECT_F rect, Card_Rank Rank) {
	int Damage = 1;
	switch (Rank)
	{
	...
	std::string text;
	text += "視点方向に7発の弾をランダムに発射し" + std::to_string(Damage * 7) + "ダメージを与え当たった対象に浸潤を3.5秒間与える。また、地面に当たるもしくは最大射程の時に下の床が紙の場合床を浸潤状態にする";
	text += "\n\n";
	text += "ランク毎の1発当たりのダメージ:" + std::to_string(DAMAGE_RANK1) + "/" + std::to_string(DAMAGE_RANK2) + "/" + std::to_string(DAMAGE_RANK3);
	SetText(rect, text);
}
void Bullet_Slug_Water::DrawPreview(D3DXVECTOR2 pos, float size, Card_Rank Rank) {

	UI ui = { pos ,D3DXVECTOR2(5.7f * size,8.9f * size),D3DXVECTOR2(0.0f,0.0f),D3DXVECTOR2(1.0f,1.0f),D3DXCOLOR(1.0f,1.0f,1.0f,1.0f) ,0.0f };
	DrawScreen(ui, RefImageMaterial().m_Card_Bullet_Slug_Water);
	if (Rank > 0)
	{
