...
{
	Draw2D_Manager::DeleateUI(m_UI_Text.GetObjectData());
...
{
	if (m_IsOpenflug) {
		m_UI_Tex.SetColer({ 0.4f,0.4f,0.4f,0.8f });
		...
		if (m_Setting == NUMBER)
		{
			...
			while (m_Text->size() > 0)
			{
				if ((m_Text->at(0) < 0x30 || m_Text->at(0) > 0x39) && (0x2E != m_Text->at(0) || 58 != m_Text->at(0)))
				{
					m_Text->erase(0, 1);
				}
				else
