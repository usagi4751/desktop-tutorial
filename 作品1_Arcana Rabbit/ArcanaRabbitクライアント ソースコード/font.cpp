...
{
   // pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF(coler)), &pSolidBrush);

	 float DEP  =  96.0f/GetDPI();
	 rect.bottom *= DEP;
	 rect.left *= DEP;
	 rect.right *= DEP;
	 rect.top *= DEP;
	...
	// 描画処理
	pRT->DrawText(wstr.c_str(), (UINT)wstr.size(), pTextFormat, rect, pSolidBrush, D2D1_DRAW_TEXT_OPTIONS_CLIP);
...
void DirectWrite::TextInit()
{
	...
	// バックバッファの取得
	// 型：IDXGISwapChain
	Renderer::GetSwapChainrend()->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));
	//gs->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));
	...
	HRESULT sa = pD2DFactory->CreateDxgiSurfaceRenderTarget(pBackBuffer, &props, &pRT);
	...
	DWRITE_FONT_WEIGHT fontWeight = DWRITE_FONT_WEIGHT::DWRITE_FONT_WEIGHT_NORMAL;;			// フォントの太さ
	DWRITE_FONT_STYLE fontStyle = DWRITE_FONT_STYLE::DWRITE_FONT_STYLE_NORMAL;;			// フォントスタイル
	DWRITE_FONT_STRETCH fontStretch = DWRITE_FONT_STRETCH::DWRITE_FONT_STRETCH_NORMAL;;		// フォントの幅

	pDWriteFactory->CreateTextFormat(L"メイリオ",
		nullptr,
		fontWeight,
		fontStyle,
		fontStretch,
	...
	pTextFormat->SetTextAlignment(DWRITE_TEXT_ALIGNMENT::DWRITE_TEXT_ALIGNMENT_LEADING);

	//関数CreateSolidColorBrush()
	//第1引数：フォント色（D2D1::ColorF(D2D1::ColorF::Black)：黒, D2D1::ColorF(D2D1::ColorF(0.0f, 0.2f, 0.9f, 1.0f))：RGBA指定）
	pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White), &pSolidBrush);
}

//=============================================================================
//     終了処理
//=============================================================================
void DirectWrite::TextRelease()
{
	// 文字描画関連のアンロード
...
void DirectWrite::SetTextSize(float size) {
...
void DirectWrite::SetTextColer(XMFLOAT4 coler)
{
	if (pSolidBrush) pSolidBrush->Release();

	pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF(coler.x, coler.y, coler.z, coler.w)), &pSolidBrush);
}
