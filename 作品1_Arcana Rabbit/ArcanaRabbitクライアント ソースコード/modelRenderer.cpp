...
void ModelRenderer::Draw(XMFLOAT4 color)
{
	// 頂点バッファ設定
	UINT stride = sizeof(VERTEX_3D);
	UINT offset = 0;
...
void ModelRenderer::LoadObj( const char *FileName, MODEL_OBJ *ModelObj )
{
	...
	while( true )
	{
		...
		else if( strcmp( str, "f" ) == 0 )
		{
			...
			while( c != '\n' && c != '\r' );
			unsigned int b = ModelObj->IndexArray[ic - 1];
			ModelObj->IndexArray[ic - 1] = ModelObj->IndexArray[ic - 2];
			ModelObj->IndexArray[ic - 2] = b;
			//四角は三角に分割
			if( in == 4 )
			{
