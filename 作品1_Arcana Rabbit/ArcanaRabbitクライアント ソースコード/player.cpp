...
void Player::Init() {
	...
	m_NowHp = Inventory::GetStayus().MaxHelth;

	m_NomalAttackRange = 5.0f;

	Manager::GetScene()->GetGameObject<Camera>()->SetTagetEnable(true);

	m_Reticl.Link(Draw2D_Manager::AddUI());
	m_Reticl.SetColer({1.0f,1.0f,1.0f,1.0f});
	m_Reticl.SetPosition({960.0f,540.0f});
	m_Reticl.SetScale({32,32});
	m_Reticl.SetTex(GetImage().m_reticle_1);
}
void Player::Uninit() {
	Draw2D_Manager::DeleateUI(m_Reticl.GetObjectData());
...
void Player::Update()
{
	...
	if (Input::IsMouseLeftTriggered() && OpenTextBoxflug == false && !m_InventoryOpen)
	{
		MouseClip(true);
	}
	m_oldpos = m_Position;
	...
	//攻撃のクールタイム更新
	m_AttackCD > 0 ? m_AttackCD -= Inventory::GetStayus().AttackSpeed: m_AttackCD = 0;
...
#endif
	////床からプレイヤーが出ないようにする
	//if (!m_floor->GetFloor({ m_Position.x , m_Position.z }))
	//{
	//	m_Position.x = m_oldpos.x;
	//	m_Position.z = m_oldpos.z;
	//}
	//最大体力を超えないようにする
	if (m_NowHp > Inventory::GetStayus().MaxHelth)
	{
		m_NowHp = Inventory::GetStayus().MaxHelth;
	}
		...
		//Manager::GetScene()->GetGameObject<ShadowVolume>()->SetPosition({ m_Position.x ,m_Position.y ,m_Position.z });
...
void Player::Action()
{
	...
	{
		MouseClip(m_InventoryOpen);
		m_InventoryOpen = !m_InventoryOpen;
	}
	//インベントリの開け閉め
	Inventory::SetOpen(m_InventoryOpen);
	...
	Inventory::SetHaveItem(m_haveitem);
}
...
void Player::AddDamage(float damage)
{
	if (damage > 0)
	{
		m_NowHp -= damage * 100.0f / (Inventory::GetStayus().Def + 100.0f);
	}
}
...
void Player::Addheal(float heal)
{
	if (heal > 0)
	{
		m_NowHp + heal > Inventory::GetStayus().MaxHelth ? m_NowHp = Inventory::GetStayus().MaxHelth : m_NowHp += heal;
	}
}
