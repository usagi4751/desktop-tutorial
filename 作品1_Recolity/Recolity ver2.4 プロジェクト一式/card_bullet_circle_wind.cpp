...
void Bullet_Circle_Wind::Use(int rank) {
	...
	GetBulletInterface()->SetNBullet(life_penetration, player, pos, D3DXVECTOR3(GetCamera()->rot.x, GetCamera()->rot.y, GetCamera()->rot.z), { 10.0f,10.0f,10.0f },
		...
		wind, element,
		&RefImageMaterial().m_Wind_Bullet, 800);
	Emitta(1, GetCamera()->pos);
}
void Bullet_Circle_Wind::DrawTextPreview(D2D1_RECT_F rect, Card_Rank Rank) {
	//カード効果説明
	int Damage = 1;
	switch (Rank)
	{
