...
#define RANGE (1500)
void Bullet_Single_Wind::Use(int rank) {
...
void Bullet_Single_Wind::DrawPreview(D3DXVECTOR2 pos, float size, Card_Rank Rank) {
	UI ui = { pos ,D3DXVECTOR2(5.7f * size,8.9f * size),D3DXVECTOR2(0.0f,0.0f),D3DXVECTOR2(1.0f,1.0f),D3DXCOLOR(1.0f,1.0f,1.0f,1.0f) ,0.0f };
	DrawScreen(ui, RefImageMaterial().m_Card_Bullet_Single_Wind);
	if (Rank > 0)
	{
...
void Bullet_Single_Wind::DrawTextPreview(D2D1_RECT_F rect, Card_Rank Rank) {
	//カード効果説明
	int Damage = 1;
	switch (Rank)
	{
	...
	std::string text;
	text += "視点方向に球を発射し" + std::to_string(Damage) + "ダメージを与え当たった対象に気流状態を与える。また、地面に当たるもしくは最大射程の時に下の床が紙の場合床を気流状態にする";
	text += "\n\n";
