...
void CustomText(std::stringstream& ss, int x, int y, D3DXVECTOR3 color, bool back)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			//文字色を白に変更
			SetTextColor(hdc, RGB((int)color.x, (int)color.y, (int)color.z));
			//背景の変更
			if (back)SetBkMode(hdc, TRANSPARENT);
			...
			//テキスト出力
			DrawText(hdc, (char*)ss.str().c_str(), (int)strlen((char*)ss.str().c_str()), &rect, DT_LEFT);
