...
void DrawSprite3DColorRotateAxis(int texNo, float X, float Y, float Z, float Width, float Height, float CX, float CY,
	float U, float V, float UW, float VH,
	float r, float g, float b, float a, float Rot)
{
	D3D11_MAPPED_SUBRESOURCE msr;
	...
	// 座標変換
	vertex[0].Position.x = (-CX - hw) * cosf(Rot) - (-CY - hh) * sinf(Rot) + X;
	vertex[0].Position.y = (-CX - hw) * sinf(Rot) + (-CY - hh) * cosf(Rot) + Y;
	vertex[0].Position.z = Z; // Z座標を設定

	vertex[1].Position.x = (-CX + hw) * cosf(Rot) - (-CY - hh) * sinf(Rot) + X;
	vertex[1].Position.y = (-CX + hw) * sinf(Rot) - (-CY - hh) * cosf(Rot) + Y;
	vertex[1].Position.z = Z;

	vertex[2].Position.x = (-CX - hw) * cosf(Rot) - (-CY + hh) * sinf(Rot) + X;
	vertex[2].Position.y = (-CX - hw) * sinf(Rot) + (-CY + hh) * cosf(Rot) + Y;
	vertex[2].Position.z = Z;

	vertex[3].Position.x = (-CX + hw) * cosf(Rot) - (-CY + hh) * sinf(Rot) + X;
	vertex[3].Position.y = (-CX + hw) * sinf(Rot) + (-CY + hh) * cosf(Rot) + Y;
	vertex[3].Position.z = Z;
