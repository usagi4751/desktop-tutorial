...
// 引数付きコンストラクタ
//=============================================================================
Boss2_Main::Boss2_Main(D3DXVECTOR3 pos, D3DXVECTOR3 rot, D3DXVECTOR3 scl, D3DXVECTOR3 vel, D3DXCOLOR color, bool use, Bullet_Interface* bif)
	...
	m_Itv_b(NORMAL_SHOT_INTERVAL), m_Itv_c(CANNON_SHOT_INTERVAL), m_BulletIF(bif)
{
	...
	attacksound1 = LoadSound("boss2_attack1");
	attacksound2 = LoadSound("boss2_attack2");
}
//=============================================================================
// デストラクタ
//=============================================================================
Boss2_Main::~Boss2_Main()
{
	Uninit();
}
//=============================================================================
...
// 終了処理
//=============================================================================
void Boss2_Main::Uninit(void)
{
	StopSound(attacksound1);
	StopSound(attacksound2);
	if (RefUse() == false)
	{
...
void Boss2_Main::Update(void)
{
	...
	//カード選択状態
	if (m_state == card_attack)
	{
		m_Card_attackcelecttime++;
		
		if (m_Card_attackcelecttime == 40)
		{
			PlaySound(attacksound1, 0, 1.0f);
		}
		if (m_Card_attackcelecttime == 150)
		{
			m_card = (CardSclect)((int)rand() % 4 + 1);
			StateSwap_Idol();
			m_card_attacktime = 0;
			PlaySound(attacksound2, 0, 1.0f);
		}
	}
	//選択後の効果
	if (m_card == FireRain)
...
void Boss2_Main::Draw(void)
{
	...
	if (m_state == card_attack && m_Card_attackcelecttime < 149)
	{
		Object carddraw;
		float size = 60.0f;
	...
	if (m_card == Ball && m_card_attacktime > 30)
	{
...
// プレイヤーとのアクション
//=============================================================================
void Boss2_Main::SearchAction(void)
{
...
void Boss2_Main::StateSwap_Move(D3DXVECTOR3 pos)
{
...
void Boss2_Main::StateSwap_Idol()
{
	m_state = idol;
}

void Boss2_Main::CardAttack()
{
